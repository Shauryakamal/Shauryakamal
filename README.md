<h1 align="center">Hi 👋, I'm Shaurya</h1>

<h3 align="center">
Java Backend Developer • Spring Boot • PostgreSQL • DSA
</h3>

<p align="center">
Building scalable backend systems with Java & Spring Boot while solving Data Structures & Algorithms every day.
</p>

<p align="center">
<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=24&duration=3000&pause=1000&center=true&vCenter=true&width=700&lines=Java+Backend+Developer;Spring+Boot+Developer;Building+Production+Ready+Projects;Daily+DSA+%7C+LeetCode;Always+Learning+Something+New+🚀"/>
</p>

---

## 👨‍💻 About Me

- 🎓 Computer Engineering Graduate
- 💻 Java Backend Developer passionate about building scalable backend applications
- 🌱 Currently learning **Spring Boot, Microservices, Docker & System Design**
- 📚 Solving **LeetCode & DSA** every day
- 🎨 Former UI/UX Designer with 2+ years of industry experience
- 🚀 Goal: Become a Backend Software Engineer at a top product-based company

---

## 🛠 Tech Stack

### Languages

<p>
<img src="https://skillicons.dev/icons?i=java,js,html,css,mysql,postgres"/>
</p>

### Backend

<p>
<img src="https://skillicons.dev/icons?i=spring"/>
</p>

**Also Familiar With**

- REST APIs
- JWT Authentication
- Maven
- Spring Data JPA / Hibernate

### Tools

<p>
<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,figma,postman"/>
</p>

---

## 🚀 Currently Working On

- 📚 Daily Data Structures & Algorithms
- ☕ Spring Boot Backend Development
- 🧠 System Design
- 🐳 Docker
- ⚙ Production Ready Backend Projects
- 🤖 AI Powered Applications

---

## 📌 Featured Projects

### 🧳 TripMind AI *(In Development)*

AI-powered travel planning platform with smart itinerary generation and expense management.

**Tech Stack**

`Java` • `Spring Boot` • `PostgreSQL` • `JWT` • `Gemini AI`

---

### 🤖 Eve AI Assistant *(In Development)*

Personal AI assistant with memory, voice interaction and intelligent task automation.

**Tech Stack**

`Python` • `Ollama` • `Gemini` • `Voice AI` • `RAG`

---

### 📚 DSA-JAVA Repository

Daily Java DSA repository containing:

- LeetCode Solutions
- Notes
- Interview Preparation
- Time Complexity Analysis
- Multiple Approaches

---

### 🧾 Ledger AI *(In Development)*

AI-powered personal finance and expense management platform.

**Tech Stack**

`Java` • `Spring Boot` • `PostgreSQL` • `AI`

---

## 📊 GitHub Stats

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Shauryakamal&show_icons=true&theme=tokyonight&hide_border=true&cache_seconds=86400"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Shauryakamal&layout=compact&theme=tokyonight&hide_border=true&cache_seconds=86400"/>

</p>

<p align="center">
<img src="https://streak-stats.demolab.com?user=Shauryakamal&theme=tokyonight&hide_border=true"/>
</p>

---

## 🎯 2026 Goals

- ✅ Solve 300+ LeetCode Problems
- ✅ Master Spring Boot
- 🔄 Learn Docker
- 🔄 Learn Microservices
- 🔄 Learn AWS
- 🔄 Master System Design
- 🎯 Switch to a Product-Based Company

---

## 🌐 Connect With Me

🌍 **Portfolio**

https://shauryaportfolio.framer.website/

💼 **LinkedIn**

https://www.linkedin.com/in/shaurya-526a73297/

💻 **LeetCode**

https://leetcode.com/u/shaurya14_02/

---

<h3 align="center">
⭐ Thanks for visiting my profile! ⭐
</h3>

<p align="center">
If you like my work, consider giving a ⭐ to my repositories.
</p>
